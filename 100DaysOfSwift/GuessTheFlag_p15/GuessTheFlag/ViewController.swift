...
class ViewController: UIViewController {
    ...
    @objc func showScore() {
        ...
        ac.addAction(UIAlertAction(title: "Thanks", style: .default, handler: askQuestion))
    ...
    @IBAction func buttonTapped(_ sender: UIButton) {
        UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.8, options: []) {
        ...
        if questionsAsked < 10 {
            ...
            let ac = UIAlertController(title: "Your final score", message: "\(score)", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "Continue", style: .default, handler: askQuestion))
            present(ac, animated: true)
            questionsAsked = 0
        }
